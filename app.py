...
# Apply compact CSS and tighten top margin
apply_compact_css()
st.markdown(
    """
    <style>
        .block-container { padding-top: 1rem; }
        .dataframe td:nth-child(3) { text-align: center !important; width: 60px; }
    </style>
    """,
...
if page == "Pick Today’s Recipe":
    ...
    if mode == "By Item Type":
        ...
        if not types:
            ...
            if selected_type and selected_type != "-- Choose --":
                ...
                filtered["Days Ago"] = filtered["Last Eaten"].apply(
                    lambda d: (today - datetime.strptime(d, "%d-%m-%Y").date()).days if pd.notna(d) else None
                )
                selected = render_selectable_table(
                    filtered,
                    select_key="by_type_select",
                    show_cols=["Recipe", "Last Eaten", "Days Ago"],
        ...
        if rec_df is None or rec_df.empty:
            ...
            rec_df["Last Eaten"] = pd.to_datetime(rec_df["Last Eaten"], errors="coerce").dt.strftime("%d-%m-%Y")
            ...
            if cols[2].button("✏️ Edit", key=f"edit_{idx}"):
                def _save_edit(new_name, new_type):
                    ...
                    try:
                        ...
                        st.error(f"Failed to save master list: {e}")
                popup_edit(f"Edit: {row['Recipe']}", row["Recipe"], row["Item Type"], _save_edit)
...
# ----------------- HISTORY -----------------
    ...
    if history_df.empty:
        ...
        if not filtered_df.empty:
            ...
            filtered_df["Date"] = pd.to_datetime(filtered_df["Date"], errors="coerce").dt.strftime("%d-%m-%Y")
